import { useNavigation } from "@react-navigation/native";
import {
  View,
  Text,
  StyleSheet,
  FlatList,
  Image,
  TouchableOpacity,
} from "react-native";

const TemplateScreen = () => {
  const navigation = useNavigation();

  const templates = [
    {
      id: 1,
      name: "Professional",
      color: "#fff",
      layout: "two-column",
    },
    {
      id: 2,
      name: "Modern",
      color: "#E5E7Eb",
      layout: "single-column",
    },
    {
      id: 3,
      name: "Classic",
      color: "#E0F2Fe",
      layout: "one-column",
    },
    {
      id: 4,
      name: "Minimalist",
      color: "#ECFDF5",
      layout: "one-column",
    },
    {
      id: 5,
      name: "Creative",
      color: "#E96790",
      layout: "two-column",
    },
    {
      id: 6,
      name: "Elegant",
      color: "#FEF3C7",
      layout: "two-column",
    },
  ];

  const renderTemplate = ({ item }: { item: any }) => {
    return (
      <View style={[styles.templateContainer, { backgroundColor: item.color }]}>
        <View style={styles.template}>
          <View style={styles.header}>
            <Image
              source={{
                uri: "data:image/jpeg;base64,",
              }}
              style={styles.avatar}
            />
            <View style={styles.headerTextContainer}>
              <Text style={styles.name}>Alex samuel</Text>
              <Text style={styles.title}>Graphic Designer</Text>
              <Text style={styles.about}>
                Alex Samuell , Creative and detail-oriented Graphic Designer
                with 4 years of experience in developing visual concepts,
                designing marketing materials, and creating brand identities.
                Proficient in Adobe Creative Suite, with a strong eye for
                typography, color, and layout. Experienced in collaborating with
                cross-functional teams to deliver impactful designs across
                digital and print platforms.
              </Text>
            </View>

            <View style={styles.columnsContainer}>
              {/*LEFT COLUMNS*/}
              <View style={styles.leftColumn}>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Profile</Text>
                  <Text style={styles.description}>
                    Creative and detail-oriented Graphic Designer
                  </Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Experience</Text>
                  <Text style={styles.date}>2019 - Present</Text>
                  <Text style={styles.subTitle}>Senior Designer</Text>
                  <Text style={styles.subText}>Creative Mind Agency</Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Eductaion</Text>
                  <Text style={styles.date}>2014 - 2019</Text>
                  <Text style={styles.subTitle}>Bachelor of Technology</Text>
                  <Text style={styles.subText}>
                    University of Engineering , London
                  </Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Certificates</Text>
                  <Text style={styles.subTitle}>
                    Adobe Creative Suite MasterClass
                  </Text>
                  <Text style={styles.subText}>Adobe training Center</Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Projects</Text>
                  <Text style={styles.date}>2021-2022</Text>
                  <Text style={styles.subTitle}>Team Lead</Text>
                  <Text style={styles.subText}>
                    Deisgned an Ecommerce Website Using Angular for s startup
                  </Text>
                </View>
              </View>

              {/*RIGHT COLUMNS*/}

              <View style={styles.rightColumn}>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Skills</Text>
                  <Text style={styles.skill}>Illustrator</Text>
                  <Text style={styles.skill}>PhotoShop</Text>
                  <Text style={styles.skill}>UI/UX design</Text>
                  <Text style={styles.skill}>3D Modelling</Text>
                </View>

                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Programming Languages</Text>
                  <Text style={styles.skill}>JAVA</Text>
                  <Text style={styles.skill}>C++</Text>
                  <Text style={styles.skill}>PYTHON</Text>
                  <Text style={styles.skill}>JAVASCRIPT</Text>
                  <Text style={styles.skill}>.NET</Text>
                </View>

                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Awards</Text>
                  <Text style={styles.subTitle}>
                    Best Graphic Designer 2022
                  </Text>
                  <Text style={styles.subText}>Creative Award</Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Hobbies</Text>
                  <Text style={styles.skill}>Photography</Text>
                  <Text style={styles.skill}>Sketching</Text>
                </View>
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>References</Text>
                  <Text style={styles.subTitle}>John Doe</Text>
                  <Text style={styles.subText}>
                    Art Director at Creative Mind
                  </Text>
                  <Text style={styles.subText}>johndoe@gmail.com</Text>
                </View>

                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Contact</Text>
                  <Text style={styles.contactText}>Phone +91- 9812345678</Text>

                  <Text style={styles.contactText}>
                    Email- example@gmail.com
                  </Text>
                </View>
              </View>
            </View>
          </View>
          <TouchableOpacity style={styles.useButton}>
            <Text style={styles.useButtonText}>Use This Template</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
  };

  return (
    <FlatList
      data={templates}
      renderItem={renderTemplate}
      keyExtractor={(item) => item.id}
      contentContainerStyle={styles.list}
    />
  );
};

const styles = StyleSheet.create({
  list: {
    padding: 20,
  },
  templateContainer: {
    borderRadius: 10,
    marginBottom: 20,
    overflow: "hidden",
  },
  template: {
    padding: 15,
  },
  header: {
    alignItems: "center",
    marginBottom: 20,
    paddingBottom: 15,
    borderBottomWidth: 1,
    borderBottomColor: "#ddd",
  },
  headerTextContainer: {
    alignItems: "center",
  },
  avatar: {
    width: 80,
    height: 80,
    borderRadius: 40,
    marginBottom: 10,
    borderWidth: 2,
    borderColor: "#007AFF",
  },
  name: {
    fontSize: 24,
    fontWeight: "bold",
    color: "#000",
    textAlign: "center",
  },
  title: {
    fontSize: 16,
    color: "#666",
    textAlign: "center",
    marginBottom: 5,
  },
  about: {
    fontSize: 14,
    color: "#333",
    textAlign: "justify",
    lineHeight: 20,
    marginBottom: 20,
  },

  columnsContainer: {
    flexDirection: "row",
    flexWrap: "wrap",
  },
  leftColumn: {
    flex: 1,
    paddingRight: 10,
    minWidth: 150,
  },
  section: {
    marginBottom: 20,
  },
  sectionTitle: {
    fontSize: 18,
    fontWeight: "bold",
    color: "#000",
    marginBottom: 8,
    borderBottomWidth: 1,
    borderBottomColor: "#007AFF",
    paddingBottom: 2,
  },
  description: {
    fontSize: 14,
    color: "#333",
    lineHeight: 20,
  },
  date: {
    fontSize: 12,
    color: "#666",
    marginBottom: 2,
  },
  subTitle: {
    fontSize: 14,
    color: "#000",
    fontWeight: 600,
  },
  subText: {
    fontSize: 13,
    color: "#666",
    marginBottom: 2,
  },
  rightColumn: {
    flex: 1,
    paddingRight: 10,
    minWidth: 150,
  },
  contactText: {
    fontSize: 14,
    color: "#333",
    marginBottom: 2,
  },
  skill: {
    fontSize: 14,
    color: "#333",
    marginBottom: 4,
  },
  useButton: {
    backgroundColor: "#007aff",
    paddingVertical: 12,
    alignItems: "center",
    borderRadius: 5,
  },
  useButtonText: {
    color: "#fff",
    fontSize: 16,
    fontWeight: 600,
  },
});

export default TemplateScreen;
